...
class RepositoryContractTests(unittest.TestCase):
    ...
    def test_cpu_visualizer_is_a_zero_dependency_learning_path(self):
        ...
        self.assertIn("See what the GPU does", html)
        self.assertIn("inside an LLM.", html)
        self.assertIn('id="mapping-view"', html)
        self.assertIn('id="inline-code"', html)
        self.assertIn('id="frame-scrubber"', html)
        self.assertIn('id="play-timeline"', html)
        self.assertIn('id="copy-lesson-link"', html)
        self.assertIn('id="llm-kernel-map"', html)
        self.assertIn("Inside one LLM token", html)
        self.assertIn("Token embedding", html)
        self.assertIn("Residual stream", html)
        self.assertIn("RMSNorm", html)
        self.assertIn("Attention softmax", html)
        self.assertIn("Q / K / V + MLP", html)
        self.assertIn("llm_examples/01_token_embedding.cu", html)
        self.assertIn("llm_examples/04_rmsnorm.cu", html)
        self.assertIn("llm_examples/06_attention_softmax.cu", html)
        self.assertIn("llm_examples/07_linear_projection.cu", html)
        self.assertIn('class="cinema-stage"', html)
        ...
        for example in ("indexing", "bounds", "vector-add", "grid-stride", "residual-add"):
        ...
        self.assertIn("GPU layer output", app)
        self.assertIn("GPU residual stream", app)
        self.assertIn("GPU updated hidden state", app)
        self.assertIn("renderExecutionMap", app)
        self.assertIn("renderMapping", app)
        self.assertIn("renderInlineCode", app)
        self.assertIn("playTimeline", app)
        ...
        self.assertIn('"residual-add"', emulator)
        self.assertIn("hidden_out[i] = layer_output[i] + residual[i]", emulator)
